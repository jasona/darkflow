...
function referencedModulePreloadPaths(html) {
  const references = [];
  const preloadPattern = /<link\b[^>]*\brel=["']modulepreload["'][^>]*\bhref=["']([^"']+)["'][^>]*>/gi;

  for (const match of html.matchAll(preloadPattern)) {
    try {
      const url = new URL(match[1], "https://client-artifact.invalid/phase0/index.html");
      if (url.origin !== "https://client-artifact.invalid" || !url.pathname.endsWith(".js")) {
        continue;
      }
      const reference = path.posix.normalize(decodeURIComponent(url.pathname)).replace(/^\/+/, "");
      if (reference !== ".." && !reference.startsWith("../")) {
        references.push(reference);
      }
    } catch {
...
async function validateRootBundle({ artifactDir, violations }) {
  const entryPath = path.join(artifactDir, "index.html");
  ...
  if (/<script\b[^>]*\bsrc=["'][^"']*\/js\/app\.js["']/i.test(html)) {
    violations.push("index.html directly loads /js/app.js");
  }
  if (/<script\b[^>]*\bsrc=["'][^"']*\.ts["']/i.test(html)) {
    violations.push("index.html directly loads a TypeScript entry");
  }

  const references = referencedJavaScriptPaths(html);
  const bundleReferences = [
    ...new Set(
      [...references, ...referencedModulePreloadPaths(html)].filter((reference) =>
        reference.startsWith("assets/"),
      ),
    ),
  ];
  const rootBundleReferences = references.filter(
    (reference) =>
      /^assets\/[A-Za-z0-9_.-]+\.js$/.test(reference) &&
      !/^assets\/phase0-/.test(reference),
  );

  if (rootBundleReferences.length === 0) {
    violations.push(
      "index.html does not reference a generated root JavaScript bundle",
    );
    return;
  }

  const existingBundleReferences = [];
  for (const reference of bundleReferences) {
    if (!reference.startsWith("assets/")) {
      continue;
    }
    if (!(await isFile(path.join(artifactDir, reference)))) {
      violations.push(`missing referenced JavaScript bundle: ${reference}`);
    } else {
      existingBundleReferences.push(reference);
    }
  }

  const bundleContents = (
    await Promise.all(
      existingBundleReferences.map((reference) =>
        fs.promises.readFile(path.join(artifactDir, reference), "utf8"),
      ),
    )
  ).join("\n");

  for (const reference of rootBundleReferences) {
    const bundlePath = path.join(artifactDir, reference);
    if (!(await isFile(bundlePath))) {
      continue;
    }

    if (!bundleContents.includes("__darkflowPhase1Bootstrap")) {
      violations.push(
        `root bundle ${reference} missing bootstrap diagnostic key`,
      );
    }
    if (!bundleContents.includes("/js/app.js")) {
      violations.push(`root bundle ${reference} missing legacy runtime handoff`);
    }
  }
}
...
async function validateClientArtifact({ artifactDir, expectedVersion }) {
  if (!path.isAbsolute(artifactDir)) {
    throw new TypeError("artifactDir must be an absolute path");
  }
  ...
  await validateRootBundle({ artifactDir, violations });
...
async function validateClientSourceParity({ artifactDir, publicDir }) {
