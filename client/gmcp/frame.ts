/** Normalized GMCP frame after package-name canonicalization and payload shaping. */
export interface GmcpFrame {
  packageName: string;
  data: unknown;
}

const CANONICAL_PACKAGES = new Map<string, string>([
  ...
  ["darkwind.tutorial.resync", "Darkwind.Tutorial.Resync"],
...
/** Resolves a raw package name to its canonical GMCP spelling. */
export function canonicalPackageName(packageName: string): string {
  const raw = typeof packageName === "string" ? packageName.trim() : "";
  return CANONICAL_PACKAGES.get(raw.toLowerCase()) ?? raw;
}

/** Normalizes Core.Supports array or object payloads to canonical package names. */
export function normalizeSupportsPayload(payload: unknown): unknown {
...
function normalizeVitals(data: unknown): unknown {
  if (!isObject(data)) {
    return data;
  }
  const out = { ...data };
  if (out.maxhp === undefined && out.mhp !== undefined) {
    out.maxhp = out.mhp;
  }
  if (out.mhp === undefined && out.maxhp !== undefined) {
    out.mhp = out.maxhp;
  }

  const mana = firstDefined(out, ["sp", "mana", "mp"]);
  const maxMana = firstDefined(out, ["maxsp", "mmana", "maxmana", "maxmp", "mmp"]);
  if (out.sp === undefined && mana !== undefined) {
    out.sp = mana;
  }
  if (out.maxsp === undefined && maxMana !== undefined) {
    out.maxsp = maxMana;
  }
  if (out.mana === undefined && mana !== undefined) {
    out.mana = mana;
  }
  if (out.mmana === undefined && maxMana !== undefined) {
    out.mmana = maxMana;
  }

  const move = firstDefined(out, ["fp", "move"]);
  const maxMove = firstDefined(out, ["maxfp", "mmove", "maxmove"]);
  if (out.fp === undefined && move !== undefined) {
    out.fp = move;
  }
  if (out.maxfp === undefined && maxMove !== undefined) {
    out.maxfp = maxMove;
  }
  if (out.move === undefined && move !== undefined) {
    out.move = move;
  }
  if (out.mmove === undefined && maxMove !== undefined) {
    out.mmove = maxMove;
  }

  if (out.string === undefined && out.hp !== undefined && out.maxhp !== undefined) {
    const parts = ["HP:" + out.hp + "/" + out.maxhp];
    if (out.sp !== undefined && out.maxsp !== undefined) {
      parts.push("SP:" + out.sp + "/" + out.maxsp);
    }
    if (out.fp !== undefined && out.maxfp !== undefined) {
      parts.push("MV:" + out.fp + "/" + out.maxfp);
    }
...
function normalizeRoomInfo(data: unknown): unknown {
  if (!isObject(data)) {
    return data;
  }
  const out = { ...data };
  if (out.environment === undefined && out.terrain !== undefined) {
    out.environment = out.terrain;
  }
  if (out.environment === undefined && out.env !== undefined) {
    out.environment = out.env;
  }
  if (out.terrain === undefined && out.environment !== undefined) {
    out.terrain = out.environment;
  }
...
function normalizeChannelMessage(data: unknown): unknown {
  if (!isObject(data)) {
    return { text: String(data ?? "") };
  }
  const out = { ...data };
  if (out.channel === undefined && out.chan !== undefined) {
    out.channel = out.chan;
  }
  if (out.chan === undefined && out.channel !== undefined) {
    out.chan = out.channel;
  }
  if (out.talker === undefined && out.player !== undefined) {
    out.talker = out.player;
  }
  if (out.player === undefined && out.talker !== undefined) {
    out.player = out.talker;
  }
  if (out.text === undefined && out.msg !== undefined) {
    out.text = out.msg;
  }
  if (out.msg === undefined && out.text !== undefined) {
    out.msg = out.text;
  }
  return out;
}

function normalizeGroup(data: unknown): unknown {
